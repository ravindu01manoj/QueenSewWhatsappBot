...
let msg = Config.LANG == 'TR' || Config.LANG == 'AZ' ? ' ' : '*This Plugin is Officially Approved!* ✅'
let unmsg = Config.LANG == 'TR' || Config.LANG == 'AZ' ? ' ' : '*This Plugin isn\'t Officially Approved!* ❌'
...
var LANG = {
            unaffinfo: Config.LANG == 'EN' || Config.LANG == 'EN' ? '*Yüklenen Pluginin Tehlike Derecesi:* _%' : '*Danger Level of Installed Plugin:* _%',
            harmful: Config.LANG == 'EN' || Config.LANG == 'EN' ? '*Bu Plugin Zararlı Olduğundan Yüklenemez!*' : '*This Plugin Cannot Be Installed As It Is Harmful!*',
            duplicate: Config.LANG == 'EN' || Config.LANG == 'EN' ? '*Aynı Plugini 2 Defa Yüklemeyezsiniz!*' : '*You Cannot Install the Same Plugin 2 Times!*',
...
QueenSew.newcmdaddtosew({pattern: 'install ?(.*)', fromMe: true, desc: Lang.INSTALL_DESC, warn: Lang.WARN}, (async (message, match) => {
    ...
    if (response.statusCode == 200) {
        ...
        try {
            ...
            fs.unlinkSync('/root/QueenSewWhatsappBot/plugins/' + plugin_name + '.js')
        ...
        if (find.includes(plugin_name)) {
            ...
            if (!match[1].includes('ravindu') && DEG.level > 99) {
                await message.client.sendMessage(message.jid,LANG.limit + DEG.level + '_', MessageType.text)
                fs.unlinkSync('/root/QueenSewWhatsappBot/plugins/' + plugin_name + '.js')
            }
            else if (!match[1].includes('ravindu') && DEG.level < 100) {
